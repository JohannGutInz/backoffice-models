...
export function TalentCard({ model }: { model: PublicModel }) {
  ...
  return (
    <Link
      ...
      className="group relative flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-black/70 shadow-lg shadow-black/40 transition-all duration-300 hover:-translate-y-1 hover:border-gold-500/40"
    >
      {/* Image area */}
      <div className="relative aspect-[3/4] overflow-hidden bg-zinc-900">
        ...
        {/* Featured badge */}
        {model.featured && (
          <span className="absolute top-2.5 right-2.5 z-10 flex items-center gap-1 rounded-full bg-gold-500/95 px-2.5 py-1 text-[10px] font-semibold text-white shadow">
        ...
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {model.categories.slice(0, 2).map((c) => (
              <span
                key={c}
                className="rounded-full bg-gold-500/15 px-2 py-0.5 text-[9.5px] font-medium tracking-wide text-gold-300 uppercase ring-1 ring-inset ring-gold-500/25"
              >
                {c}
              </span>
            ))}
